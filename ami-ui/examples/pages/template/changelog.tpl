...
<template>
  <div class="page-changelog">
    <div class="heading">
      <ami-button class="fr">
        <a href="" target="_blank">GitHub Releases</a>
      </ami-button>
      <%= 1 >
    </div>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      let fragments = `<li><h3><a href="/tag/v${release}" target="_blank">${release}</a></h3>`;
      ...
      for (let len = changeLogNodes.length, i = 2; i < len; i++) {
        ...
        if (node.tagName !== 'H3') {
          ...
          fragments += `</li><li><h3><a href="/tag/v${release}" target="_blank">${release}</a></h3>`;
        }
      }
