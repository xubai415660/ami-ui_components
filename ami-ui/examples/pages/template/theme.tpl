...
<template>
  <div class="page-container page-theme  page-component__scroll ami-scrollbar">
    ...
    <ami-dialog :visible.sync="copyDialogVisible">
      <ami-form :model="copyForm" ref="copyForm" :rules="copyFormRule">
        <ami-form-item label="<%= 3 >" prop="name">
          <ami-input v-model="copyForm.name"></ami-input>
        </ami-form-item>
      </ami-form>
      <div slot="footer" class="dialog-footer">
        <ami-button @click="closeCopyForm">{{getActionDisplayName('cancel')}}</ami-button>
        <ami-button type="primary" @click="copyToUser">{{getActionDisplayName('confirm')}}</ami-button>
      </div>
    </ami-dialog>
  </div>
</template>
