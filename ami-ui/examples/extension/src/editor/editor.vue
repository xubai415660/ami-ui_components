<template>
  <div class="ext-panel" :class="{moving : moving}" :style="{top: `${top}px`, left: `${left}px`}" ref="editor" >
    ...
    <div class="editor" :style="{height: `${height}px`}" v-show="showSidebar">
      <ami-tabs v-model="activeTab" @tab-click="onTabClick">
        <ami-tab-pane label="Config" name="config">
          <theme-configurator
        ...
        <ami-tab-pane label="Gallery" name="gallery">
          <gallery
              ref='gallery'
              :height="height"
              :width="width - 7"
...
<style>
.ext-panel .editor .ami-tabs__content, .ext-panel .editor .ami-tabs--top, .ext-panel .editor .ami-tab-pane {
  height: 100%;
}

.ext-panel .ami-tabs__nav-scroll > div {
