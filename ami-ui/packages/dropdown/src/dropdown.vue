...
import AmiButtonGroup from 'ami-ui/packages/button-group';
import {generateId} from 'ami-ui/src/utils/util';

export default {
  name: 'AmiDropdown',

  componentName: 'AmiDropdown',
  ...
  watch: {
    visible(val) {
      this.broadcast('AmiDropdownMenu', 'visible', val);
      this.$emit('visible-change', val);
    },
    focusing(val) {
      const selfDefine = this.$el.querySelector('.ami-dropdown-selfdefine');
      if (selfDefine) { // 自定义
        if (val) {
  ...
  methods: {
    ...
    initAria() {
      ...
      if (!this.splitButton) { // 自定义
        ...
        this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' ami-dropdown-selfdefine'); // 控制
      }
    },
    initEvent() {
  ...
  render(h) {
    ...
    let triggerElm = null;
    if (splitButton) {
      triggerElm = <ami-button-group>
        <ami-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick} disabled={disabled}>
          {this.$slots.default}
        </ami-button>
        <ami-button ref="trigger" type={type} size={dropdownSize} class="ami-dropdown__caret-button"
          disabled={disabled}>
          <i class="ami-dropdown__icon ami-icon-arrow-down"></i>
        </ami-button>
      </ami-button-group>;
    } else {
    ...
    return (
      <div class="ami-dropdown" v-clickoutside={hide} aria-disabled={disabled}>
        {triggerElm}
        {menuElm}
      </div>
    );
