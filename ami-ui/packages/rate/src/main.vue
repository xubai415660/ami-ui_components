  ...
  <div
    ...
    <span
      ...
      <i
          ...
          class="ami-rate__icon"
          :style="getIconStyle(item)">
        <i
            v-if="showDecimalIcon(item)"
            :class="decimalIconClass"
            :style="decimalStyle"
            class="ami-rate__decimal">
        </i>
      </i>
    </span>
    <span v-if="showText || showScore" :style="{ color: textColor }" class="ami-rate__text">{{ text }}</span>
  </div>
</template>

<script>
import {hasClass} from 'ami-ui/src/utils/dom';
import {isObject} from 'ami-ui/src/utils/types';
...
export default {
  ...
  props: {
    ...
    iconClasses: {
      type: [Array, Object],
      default() {
        return ['ami-icon-star-on', 'ami-icon-star-on', 'ami-icon-star-on'];
      }
    },
    voidIconClass: {
      type: String,
      default: 'ami-icon-star-off'
    },
    disabledVoidIconClass: {
      type: String,
      default: 'ami-icon-star-on'
    },
    disabled: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    setCurrentValue(value, event) {
      ...
      if (this.allowHalf) {
        ...
        if (hasClass(target, 'ami-rate__item')) {
          target = target.querySelector('.ami-rate__icon');
        }
        if (hasClass(target, 'ami-rate__decimal')) {
