<template>
  <div class="ami-calendar">
    <div class="ami-calendar__header">
      <div class="ami-calendar__title">
        {{ i18nDate }}
      </div>
      <div
          class="ami-calendar__button-group"
  ...
  methods: {
    ...
    rangeValidator(date, isStart) {
      ...
      if (date.getDay() !== expected) {
        console.warn('[AmiementCalendar]', message, 'Invalid range will be ignored.');
        return false;
      }
      return true;
    }
  ...
  computed: {
    ...
    validatedRange() {
      ...
      if (range.length === 2) {
        ...
        if (start > end) {
          console.warn('[AmiementCalendar]end time should be greater than start time');
          return [];
        }
        ...
        if (!validateRangeInOneMonth(startDay, end)) {
          console.warn('[AmiementCalendar]start time and end time interval must not exceed two months');
          return [];
        }
        // 第一个月的时间范围
        data.push([
