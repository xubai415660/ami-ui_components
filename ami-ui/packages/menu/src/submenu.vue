...
export default {
  ...
  computed: {
    ...
    menuTransitionName() {
      return this.rootMenu.collapse ? 'ami-zoom-in-left' : 'ami-zoom-in-top';
    },
    opened() {
    ...
    isFirstLevel() {
      ...
      while (parent && parent !== this.rootMenu) {
        if (['AmiSubmenu', 'AmiMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
          isFirstLevel = false;
          break;
        } else {
  ...
  methods: {
    ...
    handleClick() {
      ...
      this.dispatch('AmiMenu', 'submenu-click', this);
    },
    handleMouseenter(event, showTimeout = this.showTimeout) {
      ...
      this.dispatch('AmiSubmenu', 'mouse-enter-child');
    ...
    handleMouseleave(deepDispatch = false) {
      ...
      this.dispatch('AmiSubmenu', 'mouse-leave-child');
      clearTimeout(this.timeout);
      this.timeout = setTimeout(() => {
      ...
      if (this.appendToBody && deepDispatch) {
        if (this.$parent.$options.name === 'AmiSubmenu') {
  ...
  render(h) {
    ...
    const popupMenu = (
      <transition name={menuTransitionName}>
        <div
          ref="menu"
          v-show={opened}
          class={[`ami-menu--${mode}`, popperClass]}
          ...
          <ul
            role="menu"
            class={['ami-menu ami-menu--popup', `ami-menu--popup-${currentPlacement}`]}
    ...
    const inlineMenu = (
      <ami-collapse-transition>
        <ul
          role="menu"
          class="ami-menu ami-menu--inline"
          v-show={opened}
          style={{backgroundColor: rootMenu.backgroundColor || ''}}>
          {$slots.default}
        </ul>
    ...
    ) ? 'ami-icon-arrow-down' : 'ami-icon-arrow-right';
    ...
    return (
      ...
      >
        ...
        >
          {$slots.title}
          <i class={['ami-submenu__icon-arrow', submenuTitleIcon]}></i>
        </div>
