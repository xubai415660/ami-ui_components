import { arrayFindIndex } from 'ami-ui/src/utils/util';
import { getCell, getColumnByCell, getRowIdentity } from './util';
import { getStyle, hasClass, removeClass, addClass } from 'ami-ui/src/utils/dom';
import AmiCheckbox from 'ami-ui/packages/checkbox';
import AmiTooltip from 'ami-ui/packages/tooltip';
...
export default {
    ...
    return (
      <table
        ...
        <tbody>
          ...
          <ami-tooltip effect={this.table.tooltipEffect} placement="top" ref="tooltip" content={this.tooltipContent}></ami-tooltip>
        </tbody>
      </table>
    );
  },
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.ami-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      const classes = ['ami-table__row'];
      if (this.table.highlightCurrentRow && row === this.store.states.currentRow) {
        classes.push('current-row');
      }

      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('ami-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'ami-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push('ami-table__row--levami-' + treeRowData.level);
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={ this.columnsCount } class="ami-table__cell ami-table__expanded-cell">
