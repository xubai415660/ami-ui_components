<template>
  <transition name="ami-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
        v-show="visible"
        class="ami-picker-panel ami-date-range-picker ami-popper"
        :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="ami-picker-panel__body-wrapper">
        <slot class="ami-picker-panel__sidebar" name="sidebar"></slot>
        <div v-if="shortcuts" class="ami-picker-panel__sidebar">
          <button
              type="button"
              class="ami-picker-panel__shortcut"
              v-for="(shortcut, key) in shortcuts"
              :key="key"
              @click="handleShortcutClick(shortcut)">{{ shortcut.text }}
          </button>
        </div>
        <div class="ami-picker-panel__body">
          <div v-if="showTime" class="ami-date-range-picker__time-header">
            <span class="ami-date-range-picker__editors-wrap">
              <span class="ami-date-range-picker__time-picker-wrap">
                <ami-input
                    size="small"
                    :disabled="rangeState.selecting"
                    ref="minInput"
                    :placeholder="t('el.datepicker.startDate')"
                    class="ami-date-range-picker__editor"
                    :value="minVisibleDate"
                    @input="val => handleDateInput(val, 'min')"
                    @change="val => handleDateChange(val, 'min')"/>
              </span>
              <span v-clickoutside="handleMinTimeClose" class="ami-date-range-picker__time-picker-wrap">
                <ami-input
                    size="small"
                    class="ami-date-range-picker__editor"
                    :disabled="rangeState.selecting"
            ...
            <span class="ami-icon-arrow-right"></span>
            <span class="ami-date-range-picker__editors-wrap is-right">
              <span class="ami-date-range-picker__time-picker-wrap">
                <ami-input
                    size="small"
                    class="ami-date-range-picker__editor"
              ...
              <span v-clickoutside="handleMaxTimeClose" class="ami-date-range-picker__time-picker-wrap">
                <ami-input
                    size="small"
                    class="ami-date-range-picker__editor"
                    :disabled="rangeState.selecting"
          ...
          <div class="ami-picker-panel__content ami-date-range-picker__content is-left">
            <div class="ami-date-range-picker__header">
              <button
                  type="button"
                  @click="leftPrevYear"
                  class="ami-picker-panel__icon-btn ami-icon-d-arrow-left"></button>
              <button
                  type="button"
                  @click="leftPrevMonth"
                  class="ami-picker-panel__icon-btn ami-icon-arrow-left"></button>
              <button
                  type="button"
                  @click="leftNextYear"
                  v-if="unlinkPanels"
                  ...
                  class="ami-picker-panel__icon-btn ami-icon-d-arrow-right"></button>
              <button
                  type="button"
                  @click="leftNextMonth"
                  v-if="unlinkPanels"
                  ...
                  class="ami-picker-panel__icon-btn ami-icon-arrow-right"></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div class="ami-picker-panel__content ami-date-range-picker__content is-right">
            <div class="ami-date-range-picker__header">
              <button
                  type="button"
                  @click="rightPrevYear"
      ...
      <div v-if="showTime" class="ami-picker-panel__footer">
        <ami-button
            size="mini"
            type="text"
            class="ami-picker-panel__link-btn"
            @click="handleClear">
...
<script type="text/babel">
  import {
    ...
    extractDateFormat,
    extractTimeFormat
  } from 'ami-ui/src/utils/date-util';
  ...
  export default {
    ...
    methods: {
      ...
      handleRangePick(val, close = true) {
        ...
        // workaround for https://github.com/AmiemeFE/element/issues/7539, should remove this block when we don't have to care about Chromium 55 - 57
        setTimeout(() => {
          this.maxDate = maxDate;
          this.minDate = minDate;
        }, 10);
    ...
    components: {TimePicker, DateTable, AmiInput, AmiButton}
  };
</script>
