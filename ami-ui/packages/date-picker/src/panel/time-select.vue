<template>
  <transition name="ami-zoom-in-top" @before-enter="handleMenuEnter" @after-leave="$emit('dodestroy')">
    <div
        ref="popper"
        v-show="visible"
        :style="{ width: width + 'px' }"
        :class="popperClass"
        class="ami-picker-panel time-select ami-popper">
      <ami-scrollbar noresize wrap-class="ami-picker-panel__content">
...
export default {
  ...
  methods: {
    ...
    scrollToOption(selector = '.selected') {
      const menu = this.$refs.popper.querySelector('.ami-picker-panel__content');
