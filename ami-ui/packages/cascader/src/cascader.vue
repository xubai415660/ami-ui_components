<template>
  <div
    ...
    :class="[
      'ami-cascader',
      realSize && `ami-cascader--${realSize}`,
    ...
    <ami-input
      ...
      <template slot="suffix">
        <i
            ...
            key="clear"
            class="ami-input__icon ami-icon-circle-close"
    ...
    <div v-if="multiple" class="ami-cascader__tags">
      <ami-tag
      ...
      <input
          ...
          type="text"
          class="ami-cascader__search-input"
    ...
    <transition name="ami-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
          v-show="dropDownVisible"
          ref="popper"
          :class="['ami-popper', 'ami-cascader__dropdown', popperClass]">
        <ami-cascader-panel
            ref="panel"
            v-show="!filtering"
            ...
            @close="toggleDropDownVisible(false)"></ami-cascader-panel>
        <ami-scrollbar
            ref="suggestionPanel"
            v-if="filterable"
            v-show="filtering"
            tag="ul"
            class="ami-cascader__suggestion-panel"
            view-class="ami-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
                ...
                :key="item.uid"
                :class="[
                'ami-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
                :tabindex="-1"
                @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="ami-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="ami-cascader__empty-text">{{ t('el.cascader.noMatch') }}</li>
          </slot>
        </ami-scrollbar>
      </div>
    </transition>
...
import AmiCascaderPanel from 'ami-ui/packages/cascader-panel';
import AriaUtils from 'ami-ui/src/utils/aria-utils';
import {t} from 'ami-ui/src/locale';
import {isEqual, isEmpty, kebabCase} from 'ami-ui/src/utils/util';
import {isUndefined, isFunction} from 'ami-ui/src/utils/types';
import {isDef} from 'ami-ui/src/utils/shared';
...
export default {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.ami-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.ami-cascader-menu');
          firstNode = firstMenu.querySelector('.ami-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.ami-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.ami-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.ami-cascader__suggestion-list');
