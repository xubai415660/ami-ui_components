...
export default {
  ...
  methods: {
    ...
    renderEmptyText(h) {
      return (
        <div class="ami-cascader-menu__empty-text">{this.t('el.cascader.noData')}</div>
      );
    },
    renderNodeList(h) {
      const { menuId } = this;
      ...
      return [
        ...nodes,
        isHoverMenu ? <svg ref='hoverZone' class='ami-cascader-menu__hover-zone'></svg> : null
      ];
    }
  },
  ...
  render(h) {
    ...
    return (
      <ami-scrollbar
        ...
        role="menu"
        id={menuId}
        class="ami-cascader-menu"
        wrap-class="ami-cascader-menu__wrap"
        view-class={{
          'ami-cascader-menu__list': true,
          'is-empty': isEmpty
        }}
        {...events}>
