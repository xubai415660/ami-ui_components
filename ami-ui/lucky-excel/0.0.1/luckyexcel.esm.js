...
var LuckySheet = /** @class */ (function(_super) {
  ...
  LuckySheet.prototype.generateConfigDataValidations = function() {
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var _prohibitInput = getXmlAttibute(attrList, 'allowBlank', null) === '1';
      // x14 processing
...
FromEMF.Parse = function(buff, genv) {
  ...
  while (true) {
    ...
    // else if(fnm=="SETMETARGN") {}
      ...
      else if ((alg & 24) == 0) {UDOC.M.translate(gst.font.Tm, 0, gst.font.Tfs);} else {throw 'e';}
    ...
    else if (fnm == 'CLOSEFIGURE') {UDOC.G.closePath(gst);} else if (fnm == 'MOVETOEX') {
...
FromEMF._readBox = function(buff, off) {
  var b = []; for (var i = 0; i < 4; i++) {b[i] = FromEMF.B.readInt(buff, off + i * 4);} return b;
};
...
ToContext2D.prototype.PutImage = function(gst, buff, w, h, msk) {
  ...
  if (buff.length == w * h * 4) {
    ...
    if (msk && msk.length == w * h * 4) {
      for (var i = 0; i < buff.length; i += 4) {buff[i + 3] = msk[i + 1];}
    }
